...
def hutch_proxy_loss(A_t, m_diag_norm, probes=4, seed=123):
    """Hutchinson-style proxy for ||I - M^{-1}A||_F^2 via Rademacher probes.
    ...
    for p in range(probes):
        ...
        r = v - (Av * Minv)
        losses.append((r.pow(2).sum(dim=1)).mean())
    ...
    for epoch in range(1, args.epochs+1):
        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            if epoch == 1 and batch_idx < debug_print_first_n:
                ...
                print(f"[DEBUG] epoch{epoch} batch{batch_idx}: baseline losses -> ID={loss_id:.3e}, Jac={loss_jac:.3e}")
